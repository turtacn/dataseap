...
DataSeap (Datasea P) 旨在为集成了扩展检测与响应、数据安全以及安全运营/检测等先进功能的智能化安全平台，提供一个统一、高效、可扩展的数据湖底座。本项目基于 StarRocks [1] 和 Elasticsearch [2] (其中 StarRocks 为主导数据分析与查询引擎)，并结合 Pulsar 等消息队列，构建一个强大的数据处理与分析核心。DataSeap 的核心目标是简化智能化安全平台中业务对底层数据湖构建和运维的复杂度，通过汇聚来自流量、终端等多源异构的安全数据，利用先进的分析引擎和 AI 能力，实现从深度威胁检测、攻击链溯源到精准响应处置的全链路安全运营，并保障数据资产的安全、合规流转与价值释放。
...
* **高性能与实时性**: 优先保障交互式查询和实时数据分析的性能，满足安全事件的秒级响应需求。
...
* **应用服务层 (Application Service Layer)**: 包含核心的业务逻辑和服务。
    ...
    * **分析服务 (Analytics Service)**: 支持更复杂的分析场景，如安全运营的密集聚合分析、历史数据回溯扫描。该服务将深度利用StarRocks的物化视图、CBO、索引等特性。
...
数据从各类安全设备或系统（流量探针、EDR终端、日志服务器等）通过统一API网关进入数据上报服务。服务进行必要的格式校验和初步转换后，将数据作为消息发送到Pulsar集群。StarRocks通过其Routine Load或Stream Load机制（或通过Flink/Spark等中间层消费Pulsar再写入）持续地从Pulsar消费数据并加载到相应的表中。
...
* **多场景融合与隔离**:
    ...
    * 例如，为实时点查/日志检索配置高优先级、低`cpu_share`（但保证能抢占）的Workload Group；为安全运营的密集聚合分析配置中等优先级、较高`cpu_share`的Workload Group；为历史数据回溯扫描配置低优先级、受限`cpu_share`的Workload Group。
* **多任务隔离性保障**:
...
* **增强AI/ML能力集成**: 更紧密地集成机器学习框架，支持在DataSeap之上构建和运行安全领域的AI模型（如UEBA、攻击意图识别等）。
